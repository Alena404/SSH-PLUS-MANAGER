...
FTAG = b'</font>'
RESPONSE = b"HTTP/1.1 200 " + COR + MSG + FTAG + CRLF * 2
...
async def handle_connection(client_reader, client_writer):
    ...
    try:
        ...
        port = int(port_str) if port_str is not None else 22 # Default to SSH port
...
async def main():
    ...
    """
    system("clear")
    parser = argparse.ArgumentParser(description="Asyncio HTTP Proxy")
